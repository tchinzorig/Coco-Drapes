...
- **No cart / no checkout**: pricing is intentionally not shown. Customizer
  and Services submissions are emailed to the company inbox via FormSubmit.co
  (`src/utils/sendForm.js` — the destination address lives there); Careers
  submissions still show a confirmation dialog only (see Open items).
...
1. **Wire up the careers form.** The customizer quote and service request
   forms email via FormSubmit.co (`src/utils/sendForm.js`); the destination
   address must be activated once by clicking the confirmation link
   FormSubmit sends on first submission. The careers form still validates
   client-side but only shows a confirmation.
